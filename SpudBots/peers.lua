...
-- Enhanced with zoning detection and alerts
...
local ZONING_TIMEOUT_S    = 45  -- How long to keep showing zoning players (in seconds)
...
M.zoning_peers = {}     -- Stores peers that are currently zoning [id] = {data + zoning_start_time}
...
M.options = {         -- Options controlled by the main UI menu
    ...
    font_scale = 1.0,
    filler_char = "~ ~ ~ ~ ~",
    rez_alerts = true,    -- New option for rez alerts
    zoning_alerts = true, -- New option for zoning alerts
}
...
-- Zoning/Rez detection variables
local last_known_peers = {} -- Previous frame's peer list for comparison
local rez_alert_cooldown = {} -- [name] = timestamp to prevent spam
...
-- Helper: Get zoning color (pulsing orange/red)
local function getZoningColor()
    local time = os.clock()
    local pulse = math.sin(time * 4) * 0.3 + 0.7 -- Pulse between 0.4 and 1.0
    return ImVec4(1, pulse * 0.6, 0, 1) -- Orange to red pulse
end

-- Helper: Check if peer was recently seen and is now missing (zoning detection)
local function detectZoning(current_peers)
    local current_time = os.time()
    local newly_zoning = {}
    
    -- Check for peers that were present last update but missing now
    for name, last_peer in pairs(last_known_peers) do
        if name ~= MyName and not current_peers[name] then
            -- Peer was present but now missing - likely zoning
            if not M.zoning_peers[name] then
                -- First time detecting this peer as zoning
                M.zoning_peers[name] = {
                    id = name,
                    name = name,
                    server = last_peer.server or MyServer,
                    hp = last_peer.hp or 0,
                    mana = last_peer.mana or 0,
                    uses_mana = last_peer.uses_mana or false,
                    zone_id = last_peer.zone_id or 0,
                    zone = "Zoning...",
                    dps = 0,
                    aa = last_peer.aa or 0,
                    target = "None",
                    combat_state = false,
                    casting = "NULL",
                    ...
                    distance = 9999,
                    inSameZone = false,
                    class = last_peer.class or "Unknown",
                    zoning_start_time = current_time,
                    is_zoning = true
                }
                
                -- Alert for zoning (potential rez)
                if M.options.zoning_alerts then
                    print(string.format("\ay[Rez Alert] %s is zoning (possible rez taken!)\ax", name))
                end
                
                -- Add to newly zoning list for potential rez alert
                newly_zoning[name] = true
            end
        end
    end
    
    -- Clean up old zoning entries
    for name, zoning_data in pairs(M.zoning_peers) do
        if current_time - zoning_data.zoning_start_time > ZONING_TIMEOUT_S then
            M.zoning_peers[name] = nil
            print(string.format("\ag[Info] %s zoning timeout - removing from list\ax", name))
        end
    end
    
    -- Check for rez alerts (peer that was zoning now has high HP)
    if M.options.rez_alerts then
        for name, peer in pairs(current_peers) do
            if M.zoning_peers[name] and peer.hp > 50 then
                -- Peer was zoning and now has good HP - likely got rezzed
                local cooldown_key = name .. "_rez"
                if not rez_alert_cooldown[cooldown_key] or 
                   (current_time - rez_alert_cooldown[cooldown_key]) > 60 then
                    print(string.format("\ag[Rez Alert] %s has been REZZED! (HP: %.0f%%)\ax", name, peer.hp))
                    rez_alert_cooldown[cooldown_key] = current_time
                end
                -- Remove from zoning list since they're back
                M.zoning_peers[name] = nil
            end
        end
    end
...
local function refreshPeers()
    ...
    -- Store current peers for zoning detection
    local current_peers_snapshot = {}
    ...
    M.peers[my_entry_key] = self_data
    current_peers_snapshot[my_entry_key] = self_data
        ...
        if name_str ~= MyName then
            ...
            if netBot and netBot() then
                ...
                if success and peerData then
                    ...
                    current_peers_snapshot[name_str] = peerData
                    lastUpdateTime[name_str] = currentTime
                    
                    -- Remove from zoning list if they're back
                    if M.zoning_peers[name_str] then
                        M.zoning_peers[name_str] = nil
                    end
                else
    ...
    -- Detect zoning/disconnected peers
    detectZoning(current_peers_snapshot)
    
    -- Update last known peers for next comparison
    last_known_peers = current_peers_snapshot
    ...
    -- Build combined peer list (active + zoning)
    local combined_peers = {}
    for k, v in pairs(M.peers) do
        combined_peers[k] = v
    end
    for k, v in pairs(M.zoning_peers) do
        combined_peers[k] = v
    end
    ...
    local id_to_peer = combined_peers
    ...
    if M.options.sort_mode == "Custom" then
        ...
        local others = {}
        for k, p in pairs(combined_peers) do
    ...
    if M.options.sort_mode == "Class" and num_peer_rows > 0 then
        ...
        for _, peer_entry in ipairs(M.peer_list) do
            if peer_entry.type ~= "filler" then
    ...
    for _, item in ipairs(M.peer_list) do
        if item.type == "filler" then
            ...
            if first_column_is_name_or_class then
                ...
                if peer.is_zoning then
                    zoneColor = getZoningColor() -- Pulsing orange/red for zoning
                elseif isSelf then
                    zoneColor = ImVec4(1,1,0.7,1) -- Yellow for self
                elseif peer.inSameZone then
                    zoneColor = ImVec4(0.8,1,0.8,1) -- Green for same zone
                else
                    zoneColor = ImVec4(1,0.7,0.7,1) -- Red for different zone
                end
                ...
                -- Add zoning indicator to display name
                if peer.is_zoning then
                    displayValue = displayValue .. " [ZONING]"
                end
                ...
                if imgui.Selectable(uniqueLabel, false, ImGuiSelectableFlags.SpanAllColumns) then
                    if not isSelf and not peer.is_zoning then switchTo(peer.name) end
                end
                imgui.PopStyleColor()
                ...
                if imgui.IsItemHovered() then
                    ...
                    if peer.is_zoning then
                        imgui.Text("Status: ZONING/DISCONNECTED")
                        local zoning_time = os.time() - peer.zoning_start_time
                        imgui.Text("Zoning for: %d seconds", zoning_time)
                    else
                        imgui.Text("Zone: %s (%s)", peer.zone or "Unknown", peer.inSameZone and "Here" or "Elsewhere")
                    end
                    if not isSelf and not peer.is_zoning then
                        imgui.Separator()
                ...
                if not isSelf and not peer.is_zoning and imgui.IsItemClicked(ImGuiMouseButton.Right) then
            ...
            if M.options.show_hp then
                imgui.TableNextColumn()
                local hp_color = peer.is_zoning and getZoningColor() or getHealthColor(peer.hp)
                imgui.PushStyleColor(ImGuiCol.Text, hp_color)
                if peer.is_zoning then
                    imgui.Text("???")
                else
            ...
            if M.options.show_mana then
                imgui.TableNextColumn()
                if peer.is_zoning then
                    imgui.PushStyleColor(ImGuiCol.Text, getZoningColor())
                    imgui.Text("???")
                    imgui.PopStyleColor()
                elseif peer.uses_mana then
            ...
            if M.options.show_distance then
                ...
                if peer.is_zoning then
                    distText = "ZONING"; distColor = getZoningColor()
                elseif not peer.inSameZone then
                    distText = "Zone!"; distColor = ImVec4(1,0.5,0.5,1)
                elseif peer.distance == 0 and peer.name == MyName then
                    distText = "Self"; distColor = ImVec4(1,1,0.7,1)
                elseif peer.distance >= 9999 then
            ...
            if M.options.show_dps then
                ...
                if peer.is_zoning then
                    ...
                    imgui.Text(utils.cleanNumber(tonumber(peer.dps) or 0, 1, true))
                end
            end
            ...
            if M.options.show_target then
                ...
                local targetColor
                if peer.is_zoning then
                    targetColor = getZoningColor()
                elseif peer.target == "None" then
                    targetColor = ImVec4(0.7,0.7,0.7,1)
                else
                    targetColor = ImVec4(1,1,1,1)
                    if not M.options.show_combat and peer.combat_state then
                        targetColor = ImVec4(1,0.6,0.6,1) -- Red if fighting and combat col hidden
                    end
                end
                imgui.PushStyleColor(ImGuiCol.Text, targetColor)
                imgui.Text(peer.is_zoning and "---" or (peer.target or "None"))
                imgui.PopStyleColor()
            end
            ...
            if M.options.show_combat then
                ...
                if peer.is_zoning then
                    combatText = "Zoning"; combatColor = getZoningColor()
                elseif peer.combat_state then
                    combatText = "Fighting"; combatColor = ImVec4(1,0.7,0.7,1)
                else
                    combatText = "Idle"; combatColor = ImVec4(0.7,1,0.7,1)
                end
...
--[[function M.draw_aa_window()
    ...
    if imgui.Begin("Peer AA Counts", window_open_ref, ImGuiWindowFlags.NoCollapse) then
        ...
        for _, p_item in ipairs(M.peer_list) do
            if p_item.type ~= "filler" and not p_item.is_zoning then table.insert(aa_list, p_item) end
        end
...
function M.get_peer_data()
    return {
        list = M.peer_list,
        count = #M.peer_list, -- This now includes fillers and zoning peers
