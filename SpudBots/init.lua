...
local success_utils, utils = pcall(require, 'utils')
local success_peers = require('peers')
local peers = require('peers')
local success_zep, zep = pcall(require, 'Zep')
...
local function CombinedUI()
    ...
    -- Set background transparency: 0.0 = fully transparent, 1.0 = fully opaque
    local bg_alpha = 0.25
    local bg_color = imgui.GetStyleColorVec4(ImGuiCol.WindowBg)
    imgui.PushStyleColor(ImGuiCol.WindowBg, ImVec4(bg_color.x, bg_color.y, bg_color.z, bg_alpha))

    -- Begin main window
    if imgui.Begin("Spud Hub", nil, windowFlags) then
        ...
        local dpsToggled = imgui.Checkbox("Show DPS", peers.options.show_dps)
            if dpsToggled ~= peers.options.show_dps then
                peers.options.show_dps = dpsToggled
                if dpsToggled then
                    mq.cmdf('/bca //lua run spudbots')  -- Enable DPS tracking
                else
                    mq.cmdf('/bca //lua stop spudbots') -- Disable DPS tracking
                end
            end
        ...
        peers.options.rez_alerts    = imgui.Checkbox("Rez Alerts", peers.options.rez_alerts)
        peers.options.zoning_alerts = imgui.Checkbox("Zoning Alerts", peers.options.zoning_alerts)
        imgui.Separator()
        ...
        if peers.options.show_player_stats then
            DrawPlayerStats()
            -- draw a little thick separator bar
        ...
        local countPlayers = mq.TLO.SpawnCount("PC")()
        local color = countPlayers < 39 and ImVec4(1, 0.2, 0.2, 1) or ImVec4(0.7, 0.9, 1, 1)
        imgui.TextColored(color, string.format("Players In Zone (%s)", countPlayers))
        ...
        imgui.SetWindowFontScale(peers.options.font_scale or 1.0)
