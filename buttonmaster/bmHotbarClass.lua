...
function BMHotbarClass:RenderTabContextMenu()
    ...
    local deleteHotkeyPopupId = "BM_DeleteHotkey_" .. tostring(self.id)

    self.deleteHotkeySearch = self.deleteHotkeySearch or ""
    self.deleteHotkeyPopupOpen = self.deleteHotkeyPopupOpen or false
    ...
    if ImGui.BeginPopupContextItem() then
        if btnUtils.getTableSize(unassigned) > 0 then
            if ImGui.BeginMenu("Add Set") then
                local sortedUnassignedSets = {}
                for setName, _ in pairs(unassigned) do
                    table.insert(sortedUnassignedSets, setName)
                end
                table.sort(sortedUnassignedSets)

                for _, setName in ipairs(sortedUnassignedSets) do
                    if ImGui.MenuItem(setName) then
                        table.insert(BMSettings:GetCharacterWindowSets(self.id), setName)
                        BMSettings:SaveSettings(true)
                        break
                    end
                end
        ...
        local hasHotkeys = btnUtils.getTableSize(BMSettings:GetSettings().Buttons or {}) > 0
        if not hasHotkeys then ImGui.BeginDisabled(true) end
        if ImGui.MenuItem("Delete Hotkey...") and hasHotkeys then
            self.deleteHotkeySearch = ""
            self.deleteHotkeyPopupOpen = true
        end
        if not hasHotkeys then ImGui.EndDisabled() end
    ...
    if self.deleteHotkeyPopupOpen then
        ImGui.SetNextWindowSize(ImVec2(420, 360), ImGuiCond.Appearing)
        ImGui.SetNextWindowSizeConstraints(ImVec2(360, 320), ImVec2(640, 520))
        ImGui.OpenPopup(deleteHotkeyPopupId)
        self.deleteHotkeyPopupOpen = false
    end

    self:RenderDeleteHotkeyPopup(deleteHotkeyPopupId)
...
function BMHotbarClass:RenderDeleteHotkeyPopup(popupId)
    if not popupId then return end
    ...
    if ImGui.BeginPopup(popupId) then
        ImGui.Text("Delete Hotkey")
        ImGui.Separator()

        local style = ImGui.GetStyle()
        local availableWidth = ImGui.GetWindowWidth() - (style.WindowPadding.x * 2)
        local filterWidth = math.min(math.max(220, availableWidth), 600)
        ImGui.SetNextItemWidth(filterWidth)
        self.deleteHotkeySearch = select(1, ImGui.InputText("Filter Hotkeys", self.deleteHotkeySearch,
            ImGuiInputTextFlags.None))

        ImGui.Spacing()

        local listHeight = ImGui.GetTextLineHeightWithSpacing() * 10
        if ImGui.BeginChild("##delete_hotkey_list", ImVec2(0, listHeight), true) then
            local settings = BMSettings:GetSettings()

            local function buildCommandPreview(command)
                if type(command) ~= 'string' or command == '' then return '' end

                local collected = {}
                for line in command:gmatch("[^\r\n]+") do
                    table.insert(collected, line)
                    if #collected == 4 then break end
                end
                ...
                local maxLines = math.min(3, #collected)
                for i = 1, maxLines do
                    local line = collected[i] or ''
                    if #line > 140 then
                        line = line:sub(1, 140)
                        truncated = true
                    end
                ...
                local preview = table.concat(previewLines, "\n")
                if #preview > 260 then
                    preview = preview:sub(1, 260)
                    truncated = true
                end
            ...
            local sortedButtons = {}
            for buttonId, buttonData in pairs(settings.Buttons or {}) do
                local resolvedLabel = BMButtonHandlers.ResolveButtonLabel(buttonData, true)
                local commandText = type(buttonData.Cmd) == 'string' and buttonData.Cmd or ''
                table.insert(sortedButtons, {
                    Label = resolvedLabel,
                    id = buttonId,
                    command = commandText,
                    preview = buildCommandPreview(commandText),
                })
            end
            table.sort(sortedButtons, function(a, b) return (a.Label or a.id or ""):lower() < (b.Label or b.id or ""):lower() end)

            local filter = (self.deleteHotkeySearch or ""):lower()
            local matches = 0
            local totalButtons = #sortedButtons
            local tableFlags = bit32.bor(ImGuiTableFlags.RowBg, ImGuiTableFlags.BordersInnerV, ImGuiTableFlags.SizingStretchProp)

            if ImGui.BeginTable("##delete_hotkey_table", 2, tableFlags) then
                ImGui.TableSetupColumn("Hotkey", ImGuiTableColumnFlags.WidthStretch, 0.40)
                ImGui.TableSetupColumn("Command Preview", ImGuiTableColumnFlags.WidthStretch, 0.60)
                ImGui.TableHeadersRow()

                for _, buttonData in ipairs(sortedButtons) do
                    local label = buttonData.Label or buttonData.id or ""
                    local labelLower = label:lower()
                    local idLower = buttonData.id and buttonData.id:lower() or ""
                    local commandText = buttonData.command or ''
                    local commandLower = commandText:lower()
                    local previewText = buttonData.preview or ''
                    local previewLower = previewText:lower()

                    local matchesFilter = filter == "" or labelLower:find(filter, 1, true) or idLower:find(filter, 1, true)
                        or previewLower:find(filter, 1, true) or commandLower:find(filter, 1, true)

                    if matchesFilter then
                        matches = matches + 1
                        ImGui.TableNextRow()
                        ImGui.TableNextColumn()
                        local selectableLabel = string.format("%s##bm_delete_hotkey_%s", label, buttonData.id)
                        if ImGui.Selectable(selectableLabel, false) then
                            self:DeleteHotkey(buttonData.id)
                            ImGui.CloseCurrentPopup()
                            break
                        end
                        local tooltipCommand = commandText ~= '' and commandText or "No command defined"
                        if #tooltipCommand > 1000 then
                            tooltipCommand = tooltipCommand:sub(1, 1000) .. "\n..."
                        end
                        btnUtils.Tooltip(string.format("Hotkey ID: %s\nCommand:\n%s", buttonData.id, tooltipCommand))

                        ImGui.TableNextColumn()
                        if previewText ~= '' then
                            local consoleFont = ImGui.ConsoleFont
                            ...
                            if pushedFont then ImGui.PopFont() end
                        else
                            ImGui.TextDisabled("No command defined")
                        end
                    end
                end

                if totalButtons > 0 and matches == 0 then
                    ImGui.TableNextRow()
                    ImGui.TableNextColumn()
                    ImGui.TextDisabled("No hotkeys match this filter.")
                end
            ...
            if totalButtons == 0 then
                ImGui.TextDisabled("No hotkeys available.")
            end
        ...
        ImGui.Separator()
        if ImGui.Button("Close##DeleteHotkey") then
            ImGui.CloseCurrentPopup()
        end
...
function BMHotbarClass:DeleteHotkey(buttonId)
    if not buttonId then return end

    for setName, setButtons in pairs(BMSettings:GetSettings().Sets or {}) do
        for buttonKey, assignedButtonId in pairs(setButtons or {}) do
            if assignedButtonId == buttonId then
                BMSettings:GetSettings().Sets[setName][buttonKey] = nil
            end
        end
    end

    if BMSettings:GetSettings().Buttons then
