...
local function drawTickMetronome()
    ...
    if mod then
        ...
        if mod < 0.1 and now - lastFlashTime > 5000 then
            if mq.TLO.Me.CombatState() == "COMBAT" then
                mq.cmdf('/squelch /beep sounds/beep.wav')
            end
    ...
    ImGui.Text("Tracking: " .. selectedSource .. ": " .. selectedBuffName)
    ImGui.Text(string.format("Duration: %.2f s", selectedBuffDur))
    ImGui.Text(string.format("Mod 6: %.2f", selectedBuffMod))
    ImGui.Separator()
